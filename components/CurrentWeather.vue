    ...
    <div>
        <div class="flex items-end">
            <div class="flex items-start color-white font-sans mr-10">
                <span class="text-20">{{ props.weather.temperature_2m }}</span>
                <span class="text-10">{{ props.weatherUnits.temperature_2m }}</span>
            </div>

            <Icon class="color-white" name="mdi:weather-rainy" size="50" />
        </div>

        <div class="my-4">
            <p class="font-sans text-10 color-white">{{ location?.name }}</p>
        </div>

        <div class="flex justify-start items-center color-white">
            <div class="flex flex-col items-center mr-4">
               <Icon class="color-white mb-1" name="mdi:weather-windy" size="20" />
               <p>{{ props.weather.wind_speed_10m + props.weatherUnits.wind_speed_10m }}</p>
            </div>

            <div class="flex flex-col items-center">
               <Icon class="color-white mb-1" name="mdi:water" size="20" />
               <p>{{ props.weather.relative_humidity_2m + props.weatherUnits.relative_humidity_2m }}</p>
            </div>
        </div>
    </div>
</template>